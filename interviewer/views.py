...
def Interviewer_Interview_List(request):
    context = {
            'int': Interview_Interviewer.objects.filter(Interviewer=request.user),
            'topic': 'Interviews',
...
def Interviewer_Cv_list(request,iid):
    ...
    if request.POST:
        ...
        if interviewer_form.is_valid():
            l = interviewer_form.cleaned_data['Interviewer_Review']
            a = Interview.objects.get(id = iid)
            a.Interviewer_Review = l
            a.save()
